...
rss_url = f"https://www.youtube.com/feeds/videos.xml?channel_id="

for channel_id in YOUTUBE_CHANNEL_IDS:
    response = requests.get(rss_url + channel_id) # Send an HTTP GET request

if response.status_code == 200:
    print("Successfully fetched the RSS feed!")
    print(response.text) # raw RSS feed content
else:
    print(f"Error: Unable to fetch RSS feed. Status code: {response.status_code}")
